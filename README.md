...
Since this project excludes large binaries and build artifacts via `.gitignore`, you need to initialize the environment after cloning:

1. **Install Dependencies**:
   Ensure you have Python 3.9+ and Xcode installed.

2. **Build Bundled Components**:
   Run the following scripts to generate the necessary binaries:
   ```bash
   # Build the DVT location stream binary
   ./scripts/build-dvt-stream.sh

   # Setup the bundled pymobiledevice3 environment
   ./scripts/build-bundled-pymobiledevice3.sh
   ```

3. **Build & Run**:
   Open `flyflyfly.xcodeproj` in Xcode or use the provided runner:
   ```bash
   ./runfly.sh
   ```
