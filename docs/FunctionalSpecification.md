...
*   **輸入參數**：
    *   起點/終點座標。
    *   模擬移動速度：`0.1 km/h` 至 `300.0 km/h`（步行、騎行、開車與高速測試速度動態微調）。
...
*   **操作回饋**：HUD 面板實時更新「當前進度 %」、「剩餘總里程 (km)」與「預估剩餘時間 (hh:mm:ss)」，支援「停止模擬 / 繼續模擬」控制。
...
*   **沙盒相容 Native 實作**：
    - 修復流程完全不使用任何 `/bin/bash` 外部進程或需要 root 權限的 killall 操作（避免安全提權漏洞）。連線憑證轉換流程會以受限方式呼叫系統 `/usr/bin/openssl` 產生臨時 p12，完成後立即清除暫存憑證檔。
...
graph TD
    ...
    SelectDest -->|定點跳轉| Fixed[6a. 地圖左鍵點選 / 檢索目的地 -> 立即定位]
    SelectDest -->|路徑/多點| Route[6b. 規劃起點/終點 -> 設定速度 -> 設定循環模式 -> 點擊 開始定位]
    
    Fixed --> HUD[7a. 側邊欄顯示定位狀態與停止定位控制]
    Route --> RouteHUD[7b. HUD 面板實時顯示軌跡進度與剩餘里程]
    HUD --> Stop[8. 點擊「停止定位 / 停止模擬」或拔除 USB, 設備自動還原定位]
    RouteHUD --> Stop
```
...
### 🗺️ 2.4 定點與軌跡模擬執行流程
1.  **選擇跳轉點**：
    - 方法 A：在地圖的任何位置點選滑鼠左鍵，即會將該座標送入目前操作模式。
    - 方法 B：在頂部搜尋框輸入地址（如 `台北101`），搜尋完成後點選匹配推薦，地圖會自動對焦並定位。
2.  **定點直接跳轉**：
    - 切換至定點定位模式後，在地圖上任意點擊即可直接開始或更新定位；側邊欄主按鈕會顯示 **停止定位**，用於停止定點定位並清除裝置上的模擬位置。
3.  **擬真路徑/巡邏漫步**：
    - 在側邊欄切換為「路徑模式」或「多點路徑」。
    - 在地圖上點選多個導航途經點。
    ...
    - 點擊 **開始定位** 按鈕。
4.  **監控與還原**：
    - 路徑/多點模式下，HUD 控制面板將以擬真圓環與進度條即時更新當前經緯度、速度、已走距離與剩餘時間；定點模式則以側邊欄狀態與 **停止定位** 控制為主。
    - 模擬過程中，可隨時點擊 **停止模擬**；停止後可透過主按鈕 **繼續模擬**。
    - 點擊 **停止定位 / 停止模擬** 或清除目前路線時，App 會透過 DTX path 發送 `stopLocationSimulation`，iOS 設備會立即安全還原至物理真實世界的 GPS 定位。
...
2.  **macOS Sandbox 完全相容**：
    - 本應用不以 bash 腳本或 root 權限執行修復流程；必要的憑證格式轉換會呼叫系統 `/usr/bin/openssl`，輸入輸出限制在 App 建立的臨時憑證檔，並於匯入 Keychain 前後清除。
